...
const handler: TNextApiHandler<ICommentReactionResponse, ICommentReactionBody, {}> = async (req, res) => {
	...
	if (!commentDoc || !commentDoc.exists || !commentDoc.data()) {
		return res.status(StatusCodes.NOT_FOUND).json({ error: messages.UNABLE_TO_FIND_COMMENT_FOR_POST(comment_id,post_id,room_id,house_id) });
	}

	if (commentDoc.data()?.is_deleted) {
		return res.status(StatusCodes.NOT_FOUND).json({ error: messages.UNABLE_TO_ADD_REACTION(comment_id) });
	}
