...
const handler: TNextApiHandler<ICommentReactionResponse, ICommentReactionBody, {}> = async (req, res) => {
	...
	const { type, proposal_id, room_id, house_id, comment_id } = req.body;
	...
	if (!commentDoc || !commentDoc.exists || !commentDoc.data()) {
		return res.status(StatusCodes.NOT_FOUND).json({ error: `Comment with id "${comment_id}" is not found for a Proposal with id "${proposal_id}" in a Room with id "${room_id}" and a House with id "${house_id}".` });
	}

	if (commentDoc.data()?.is_deleted) {
		return res.status(StatusCodes.NOT_FOUND).json({ error: `Unable to add the reaction for a Comment with id "${comment_id}" because it is deleted.` });
	}
