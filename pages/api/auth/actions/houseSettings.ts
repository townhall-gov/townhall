...
const handler: TNextApiHandler<IHouseSettingsResponse, IHouseSettingsBody, {}> = async (req, res) => {
	...
	if (!houseRefDoc || !houseRefDoc.exists || !houseRefDoc.data()) {
		return res.status(StatusCodes.NOT_FOUND).json({ error: messages.HOUSE_DOES_NOT_EXIST(houseId) });
	}
	...
	if (!isUserAdmin) {
		return res.status(StatusCodes.FORBIDDEN).json({ error: messages.ONLY_HOUSE_ADMIN_CAN_EDIT_HOUSE_SETTINGS });
	}
