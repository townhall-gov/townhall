...
import { useSelectedComments } from '~src/redux/discussion/selectors';

const CommentsWrapper = () => {
	const select = 5;
	const { comments, total } = useSelectedComments(select);
	const { isAllCommentsVisible } = useDiscussionSelector();
			...
			}
			{
				total > select?
					<div className='flex items-center justify-center border border-solid border-transparent border-b-[#373737] pb-1 drop-shadow-[0px_-9px_14px_#66A5FF]'>
						<button
							onClick={() => {
								dispatch(discussionActions.setIsAllCommentsVisible(!isAllCommentsVisible));
							}}
