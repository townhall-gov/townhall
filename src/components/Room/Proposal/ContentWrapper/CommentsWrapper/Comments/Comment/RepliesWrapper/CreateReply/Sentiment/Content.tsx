...
const SentimentModalContent = () => {
	const { editableReply } = useProposalSelector();
	const selectedSentiment:any = editableReply?.id? editableReply.sentiment || editableReply.sentiment: editableReply?.sentiment;
		...
		<section>
			...
			<article className='mt-7 flex flex-col items-center justify-center'>
				...
				<h4 className='m-0 text-purple_primary font-semibold text-xl leading-6 tracking-[0.01em]'>
					{(selectedSentiment || '')?.toString().split('_').map((str:any) => str.charAt(0)?.toUpperCase() + str.slice(1).toLowerCase()).join(' ')}
				</h4>
			</article>
		</section>
	);
